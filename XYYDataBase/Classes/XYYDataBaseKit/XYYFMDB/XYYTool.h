 ...
 */
#ifdef DEBUG
#define xyy_log(...) NSLog(__VA_ARGS__)
#else
#define xyy_log(...)
#endif

#define xyy_completeBlock(obj) !complete?:complete(obj);

#define XYY @"XYY_"
#define xyy_tableNameKey @"xyy_tableName"
#define xyy_rowid @"rowid"

#define xyy_uniqueKeysSelector NSSelectorFromString(@"xyy_uniqueKeys")
#define xyy_ignoreKeysSelector NSSelectorFromString(@"xyy_ignoreKeys")
#define xyy_unionPrimaryKeysSelector NSSelectorFromString(@"xyy_unionPrimaryKeys")

typedef NS_ENUM(NSInteger,xyy_getModelInfoType){//过滤数据类型
    xyy_ModelInfoInsert,//插入过滤
    xyy_ModelInfoSingleUpdate,//单条更新过滤
    xyy_ModelInfoArrayUpdate,//批量更新过滤
    xyy_ModelInfoNone//无过滤
};

@interface XYYTool : NSObject
/**
 json字符转json格式数据 .
 */
 ...
 */
+(id _Nonnull)xyy_objectWithClass:(__unsafe_unretained _Nonnull Class)cla value:(id _Nonnull)value;
/**
 模型转字典.
 */
+(NSMutableDictionary* _Nonnull)xyy_keyValuesWithObject:(id _Nonnull)object ignoredKeys:(NSArray* _Nullable)ignoredKeys;
/**
 判断并执行类方法.
 */
...
/**
 过滤建表的key.
 */
+(NSArray* _Nonnull)xyy_filtCreateKeys:(NSArray* _Nonnull)createkeys ignoredkeys:(NSArray* _Nonnull)ignoredkeys;
/**
 如果表格不存在就新建.
 */
