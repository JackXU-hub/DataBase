...
#define XYYValue @"XYYValue"
#define XYYData @"XYYData"
#define XYYArray @"XYYArray"
#define XYYSet @"XYYSet"
#define XYYDictionary @"XYYDictionary"
#define XYYModel @"XYYModel"
#define XYYMapTable @"XYYMapTable"
#define XYYHashTable @"XYYHashTable"

#define xyy_typeHead_NS @"@\"NS"
#define xyy_typeHead__NS @"@\"__NS"

#define xyy_typeHead_UI @"@\"UI"
#define xyy_typeHead__UI @"@\"__UI"
 ...
 *  遍历所有类的block（父类）
 */
typedef void (^XYYClassesEnumeration)(Class c, BOOL *stop);
static NSSet *foundationClasses_;

@implementation XYYTool
/**
 封装处理传入数据库的key和value.
 */
NSString* xyy_sqlKey(NSString* key){
    return [NSString stringWithFormat:@"%@%@",XYY,key];
}

/**
 转换OC对象成数据库数据.
 */
NSString* xyy_sqlValue(id value){
    ...
    if([value isKindOfClass:[NSNumber class]]) {
        ...
        value = [XYYTool getSqlValue:value type:type encode:YES];
 ...
 */
NSString* xyy_keyPathValues(NSArray* keyPathValues){
    return [XYYTool getLikeWithKeyPathAndValues:keyPathValues where:NO];
}
/**
 自定义数据库名称.
 */
void xyy_setSqliteName(NSString*_Nonnull sqliteName){
    if (![sqliteName isEqualToString:[XYYDB shareManager].sqliteName]) {
        [XYYDB shareManager].sqliteName = sqliteName;
    }
}
/**
 删除数据库文件
 */
BOOL xyy_deleteSqlite(NSString*_Nonnull sqliteName){
    return [XYYDB deleteSqlite:sqliteName];
}
/**
 设置操作过程中不可关闭数据库(即closeDB函数无效).
 默认是NO.
 */
void xyy_setDisableCloseDB(BOOL disableCloseDB){
    if ([XYYDB shareManager].disableCloseDB != disableCloseDB){//防止重复设置.
        [XYYDB shareManager].disableCloseDB = disableCloseDB;
    }
}
/**
 手动关闭数据库.
 */
void xyy_closeDB(){
    BOOL closeFlag = [XYYDB shareManager].disableCloseDB;
    [XYYDB shareManager].disableCloseDB = NO;
    [[XYYDB shareManager] closeDB];
    [XYYDB shareManager].disableCloseDB = closeFlag;
}
/**
 设置调试模式
 @debug YES:打印调试信息, NO:不打印调试信息.
 */
void xyy_setDebug(BOOL debug){
    if ([XYYDB shareManager].debug != debug){//防止重复设置.
        [XYYDB shareManager].debug = debug;
    }
}
 ...
 */
void xyy_inTransaction(BOOL (^ _Nonnull block)()){
    [[XYYDB shareManager] inTransaction:block];
}
/**
 清除缓存
 */
void xyy_cleanCache(){
    [[NSCache xyy_cache] removeAllObjects];
}
/**
 json字符转json格式数据 .
 */
...
+ (void)xyy_enumerateClasses:(__unsafe_unretained Class)srcCla complete:(XYYClassesEnumeration)enumeration
{
    // 1.没有block就直接返回
...
+(NSArray*)getClassIvarList:(__unsafe_unretained Class)cla Object:(_Nullable id)object onlyKey:(BOOL)onlyKey{
    ...
    if(onlyKey){
        [keys addObject:xyy_primaryKey];
        [keys addObject:xyy_createTimeKey];
        [keys addObject:xyy_updateTimeKey];
    }else{
        //手动添加库自带的自动增长主键ID和类型q
        [keys addObject:[NSString stringWithFormat:@"%@*q",xyy_primaryKey]];
        //建表时此处加入额外的两个字段(createTime和updateTime).
        [keys addObject:[NSString stringWithFormat:@"%@*@\"NSString\"",xyy_createTimeKey]];
        [keys addObject:[NSString stringWithFormat:@"%@*@\"NSString\"",xyy_updateTimeKey]];
    }
    
    [self xyy_enumerateClasses:cla complete:^(__unsafe_unretained Class c, BOOL *stop) {
...
+(NSArray*)where:(NSArray*)where{
    ...
    if(!(where.count%3)){
        ...
        for(int i=0;i<where.count;i+=3){
            [SQL appendFormat:@"%@%@%@?",XYY,where[i],where[i+1]];
            if (i != (where.count-3)) {
...
+(NSString*)getLikeWithKeyPathAndValues:(NSArray* _Nonnull)keyPathValues where:(BOOL)where{
    ...
    for(int i=0;i<keysCount;i++){
        ...
        if ([relations[i] isEqualToString:xyy_contains]){//包含关系
            if(![value isKindOfClass:[NSString class]]){
        ...
        if(likeOr){
            if(keypaths.count<=2){
                [likeM appendFormat:@"((%@%@ like '%@') or (%@%@ like '%@'))",XYY,keypaths[0],keyPathParam,XYY,keypaths[0],[keyPathParam stringByReplacingOccurrencesOfString:@"," withString:@"\n}"]];
            }else{
                [likeM appendFormat:@"((%@%@ like '%@') or (%@%@ like '%@'))",XYY,keypaths[0],keyPathParam,XYY,keypaths[0],[keyPathParam stringByReplacingOccurrencesOfString:@"," withString:@"\\"]];
            }
        }else{
            [likeM appendFormat:@"%@%@ like '%@'",XYY,keypaths[0],keyPathParam];
        }
        if(i != (keys.count-1)){
...
+(NSString*)keyAndType:(NSString*)param{
    ...
    //设置列名(XYY + 属性名),加XYY_是为了防止和数据库关键字发生冲突.
    return [NSString stringWithFormat:@"%@ %@",[NSString stringWithFormat:@"%@%@",XYY,key],SqlType];
}
...
+(NSString *)jsonStringWithObject:(id)object{
    ...
    //忽略属性
    NSArray* ignoreKeys = [XYYTool executeSelector:xyy_ignoreKeysSelector forClass:[object class]];
    for(NSString* keyAndType in keyAndTypes){
        ...
        if(![propertyName isEqualToString:xyy_primaryKey]){
...
+(NSDictionary*)dictionaryForArrayInsert:(id)value{
    ...
    if ([value isKindOfClass:[NSArray class]]){
        return @{XYYArray:[self jsonStringWithArray:value]};
    }else if ([value isKindOfClass:[NSSet class]]){
        return @{XYYSet:[self jsonStringWithArray:value]};
    }else if ([value isKindOfClass:[NSString class]] || [value isKindOfClass:[NSNumber class]]){
        return @{XYYValue:value};
    }else if([value isKindOfClass:[NSData class]]){
        NSData* data = value;
        NSNumber* maxLength = MaxData;
        NSAssert(data.length<maxLength.integerValue,@"最大存储限制为100M");
        return @{XYYData:[value base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength]};
    }else if ([value isKindOfClass:[NSDictionary class]]){
        return @{XYYDictionary:[self jsonStringWithDictionary:value]};
    }else if ([value isKindOfClass:[NSMapTable class]]){
        return @{XYYMapTable:[self jsonStringWithMapTable:value]};
    }else if([value isKindOfClass:[NSHashTable class]]){
        return @{XYYHashTable:[self jsonStringWithNSHashTable:value]};
    }else{
        NSString* modelKey = [NSString stringWithFormat:@"%@*%@",XYYModel,NSStringFromClass([value class])];
...
+(id)getSqlValue:(id)value type:(NSString*)type encode:(BOOL)encode{
    ...
    if(([type hasPrefix:xyy_typeHead_NS]||[type hasPrefix:xyy_typeHead__NS])&&[type containsString:@"String"]){
    ...
    }else if(([type hasPrefix:xyy_typeHead_NS]||[type hasPrefix:xyy_typeHead__NS])&&[type containsString:@"Number"]){
        if(encode) {
            return [NSString stringWithFormat:@"%@",value];
        }else{
            return [[NSNumberFormatter new] numberFromString:value];
        }
    }else if(([type hasPrefix:xyy_typeHead_NS]||[type hasPrefix:xyy_typeHead__NS])&&[type containsString:@"Array"]){
        if(encode){
            return [self jsonStringWithArray:value];
        }else{
            return [self arrayFromJsonString:value];
        }
    }else if(([type hasPrefix:xyy_typeHead_NS]||[type hasPrefix:xyy_typeHead__NS])&&[type containsString:@"Dictionary"]){
        if(encode){
            return [self jsonStringWithDictionary:value];
        }else{
            return [self dictionaryFromJsonString:value];
        }
    }else if(([type hasPrefix:xyy_typeHead_NS]||[type hasPrefix:xyy_typeHead__NS])&&[type containsString:@"Set"]){
        if(encode){
            return [self jsonStringWithArray:value];
        }else{
            return [self arrayFromJsonString:value];
        }
    }else if(([type hasPrefix:xyy_typeHead_NS]||[type hasPrefix:xyy_typeHead__NS])&&[type containsString:@"Data"]){
        if(encode){
            NSData* data = value;
            NSNumber* maxLength = MaxData;
            NSAssert(data.length<maxLength.integerValue,@"最大存储限制为100M");
    ...
    }else if(([type hasPrefix:xyy_typeHead_NS]||[type hasPrefix:xyy_typeHead__NS])&&[type containsString:@"MapTable"]){
        if(encode){
            return [self jsonStringWithMapTable:value];
        }else{
            return [self mapTableFromJsonString:value];
        }
    }else if(([type hasPrefix:xyy_typeHead_NS]||[type hasPrefix:xyy_typeHead__NS])&&[type containsString:@"HashTable"]){
        if(encode){
            return [self jsonStringWithNSHashTable:value];
        }else{
            return [self hashTableFromJsonString:value];
        }
    }else if(([type hasPrefix:xyy_typeHead_NS]||[type hasPrefix:xyy_typeHead__NS])&&[type containsString:@"Date"]){
        if(encode){
            return [self stringWithDate:value];
        }else{
            return [self dateFromString:value];
        }
    }else if(([type hasPrefix:xyy_typeHead_NS]||[type hasPrefix:xyy_typeHead__NS])&&[type containsString:@"URL"]){
        if(encode){
            return [value absoluteString];
        }else{
            return [NSURL URLWithString:value];
        }
    }else if(([type hasPrefix:xyy_typeHead_UI]||[type hasPrefix:xyy_typeHead__UI])&&[type containsString:@"Image"]){
        if(encode){
    ...
    }else if(([type hasPrefix:xyy_typeHead_UI]||[type hasPrefix:xyy_typeHead__UI])&&[type containsString:@"Color"]){
        if(encode){
            CGFloat r, g, b, a;
        ...
        if(encode){
            ...
            if([value containsString:XYYModel]){//自定义的类
...
+(id)objectFromJsonStringWithTableName:(NSString* _Nonnull)tablename class:(__unsafe_unretained _Nonnull Class)cla valueDict:(NSDictionary*)valueDict{
    ...
    for(int i=0;i<valueDictKeys.count;i++){
        ...
        if([sqlKey containsString:XYY]){
            tempSqlKey = [sqlKey stringByReplacingOccurrencesOfString:XYY withString:@""];
        }
        for(NSString* keyAndType in keyAndTypes){
    ...
    [object setValue:tablename forKey:xyy_tableNameKey];
 ...
 */
+(id)xyy_objectWithClass:(__unsafe_unretained _Nonnull Class)cla value:(id)value{
    ...
    NSDictionary* const objectClaInArr = [XYYTool executeSelector:NSSelectorFromString(@"xyy_objectClassInArray") forClass:[object class]];
    NSDictionary* const objectClaForCustom = [XYYTool executeSelector:NSSelectorFromString(@"xyy_objectClassForCustom") forClass:[object class]];
    NSDictionary* const xyy_replacedKeyFromPropertyNameDict = [XYYTool executeSelector:NSSelectorFromString(@"xyy_replacedKeyFromPropertyName") forClass:[object class]];
    ...
    //处理要替换的key和属性名.
    if(xyy_replacedKeyFromPropertyNameDict && xyy_replacedKeyFromPropertyNameDict.count){
        [xyy_replacedKeyFromPropertyNameDict enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
            ...
            if ([key isEqualToString:claKey]){
                ...
                !objectClaInArr?:[objectClaInArr enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull Arrkey, id  _Nonnull ArrObjCla, BOOL * _Nonnull stop){
                    if([key isEqualToString:Arrkey]){
                        ...
                        for(NSDictionary* ArrObj in dataDictObj){
                            id obj = [self xyy_objectWithClass:ArrObjCla value:ArrObj];
                ...
                !objectClaForCustom?:[objectClaForCustom enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull customKey, id  _Nonnull customObj, BOOL * _Nonnull stop) {
                    ...
                    if([key isEqualToString:tempKey]){
                        ArrObject = [self xyy_objectWithClass:customObj value:[dataDict valueForKey:tempKey]];
                        *stop = YES;
                    }
                }];
...
/**
 模型转字典.
 */
+(NSMutableDictionary*)xyy_keyValuesWithObject:(id)object ignoredKeys:(NSArray*)ignoredKeys{
    ...
    NSDictionary* const objectClaInArr = [XYYTool executeSelector:NSSelectorFromString(@"xyy_objectClassInArray") forClass:[object class]];
    NSDictionary* const objectClaForCustom = [XYYTool executeSelector:NSSelectorFromString(@"xyy_dictForCustomClass") forClass:[object class]];
    NSMutableDictionary* dictM = [NSMutableDictionary dictionary];
    ...
    [keys enumerateObjectsUsingBlock:^(NSString * _Nonnull key, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        !objectClaInArr?:[objectClaInArr enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull Arrkey, id  _Nonnull ArrObjCla, BOOL * _Nonnull stop){
            if([key isEqualToString:Arrkey]){
                ...
                for(id arrObj in value){
                    id dictObj = [self xyy_keyValuesWithObject:arrObj ignoredKeys:nil];
        ...
        !objectClaForCustom?:[objectClaForCustom enumerateKeysAndObjectsUsingBlock:^(NSString*  _Nonnull customKey, id  _Nonnull customObj, BOOL * _Nonnull stop) {
            if([key isEqualToString:customKey]){
                value = [self xyy_keyValuesWithObject:[object valueForKey:customKey] ignoredKeys:nil];
                *stop = YES;
            }
        }];
...
+(NSArray*)arrayFromJsonString:(NSString*)jsonString{
    ...
    if([jsonString containsString:XYYModel] || [jsonString containsString:XYYData]){
        NSMutableArray* arrM = [NSMutableArray array];
...
//                if ([obj containsString:XYYModel]) {
...
//        }];
        ...
        id object = [XYYTool objectFromJsonStringWithTableName:tableName class:cla valueDict:dict];
...
+(NSMutableDictionary*)getDictWithObject:(id)object ignoredKeys:(NSArray* const)ignoredKeys{
    ...
    for(NSString* keyAndType in keyAndTypes){
        ...
        if(![ignoredKeys containsObject:propertyName]){
            ...
            //数据库表列名(XYY_ + 属性名),加XYY_是为了防止和数据库关键字发生冲突.
            NSString* sqlColumnName = [NSString stringWithFormat:@"%@%@",XYY,propertyName];
            
            id propertyValue;
            id sqlValue;
            //crateTime和updateTime两个额外字段单独处理.
            if([propertyName isEqualToString:xyy_createTimeKey] ||
               [propertyName isEqualToString:xyy_updateTimeKey]){
                propertyValue = [XYYTool stringWithDate:[NSDate new]];
            }else{
            ...
            if(propertyValue){
                //列值
                sqlValue = [XYYTool getSqlValue:propertyValue type:propertyType encode:YES];
...
/**
 根据对象获取要更新或插入的字典.
 */
+(NSDictionary* _Nonnull)getDictWithObject:(id _Nonnull)object ignoredKeys:(NSArray* const _Nullable)ignoredKeys filtModelInfoType:(xyy_getModelInfoType)filtModelInfoType{
    
    //获取存到数据库的数据.
    NSMutableDictionary* valueDict = [self getDictWithObject:object ignoredKeys:ignoredKeys];
    
    if (filtModelInfoType == xyy_ModelInfoSingleUpdate){//单条更新操作时,移除 创建时间和主键 字段不做更新
        [valueDict removeObjectForKey:xyy_sqlKey(xyy_createTimeKey)];
        //判断是否定义了“联合主键”.
        NSArray* unionPrimaryKeys = [XYYTool executeSelector:xyy_unionPrimaryKeysSelector forClass:[object class]];
        NSString* xyy_id = xyy_sqlKey(xyy_primaryKey);
        if(unionPrimaryKeys.count == 0){
            if([valueDict.allKeys containsObject:xyy_id]) {
                [valueDict removeObjectForKey:xyy_id];
            }
        }else{
            if(![valueDict.allKeys containsObject:xyy_id]) {
                valueDict[xyy_id] = @(1);//没有就预备放入
            }
        }
    }else if(filtModelInfoType == xyy_ModelInfoInsert){//插入时要移除主键,不然会出错.
        //判断是否定义了“联合主键”.
        NSArray* unionPrimaryKeys = [XYYTool executeSelector:xyy_unionPrimaryKeysSelector forClass:[object class]];
        NSString* xyy_id = xyy_sqlKey(xyy_primaryKey);
        if(unionPrimaryKeys.count == 0){
    ...
    }else if(filtModelInfoType == xyy_ModelInfoArrayUpdate){//批量更新操作时,移除 创建时间 字段不做更新
        [valueDict removeObjectForKey:xyy_sqlKey(xyy_createTimeKey)];
    }else;
...
/**
 过滤建表的key.
 */
+(NSArray*)xyy_filtCreateKeys:(NSArray*)xyy_createkeys ignoredkeys:(NSArray*)xyy_ignoredkeys{
    NSMutableArray* createKeys = [NSMutableArray arrayWithArray:xyy_createkeys];
    NSMutableArray* ignoredKeys = [NSMutableArray arrayWithArray:xyy_ignoredkeys];
    //判断是否有需要忽略的key集合.
...
+(BOOL)ifNotExistWillCreateTableWithObject:(id)object ignoredKeys:(NSArray* const)ignoredKeys{
    ...
    //获取"唯一约束"字段名
    NSArray* uniqueKeys = [XYYTool executeSelector:xyy_uniqueKeysSelector forClass:[object class]];
    //获取“联合主键”字段名
    NSArray* unionPrimaryKeys = [XYYTool executeSelector:xyy_unionPrimaryKeysSelector forClass:[object class]];
    __block BOOL isExistTable;
    [[XYYDB shareManager] isExistWithTableName:tableName complete:^(BOOL isExist) {
        if (!isExist){//如果不存在就新建
            NSArray* createKeys = [self xyy_filtCreateKeys:[XYYTool getClassIvarList:[object class] Object:object onlyKey:NO] ignoredkeys:ignoredKeys];
            [[XYYDB shareManager] createTableWithTableName:tableName keys:createKeys unionPrimaryKeys:unionPrimaryKeys uniqueKeys:uniqueKeys complete:^(BOOL isSuccess) {
